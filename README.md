<div align="center">

![Header](https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=12,14,18,20,24&height=200&section=header&text=Muhammad%20Hamka&fontSize=50&fontColor=fff&animation=twinkling&fontAlignY=35&desc=Frontend%20Developer%20%7C%20Tech%20Enthusiast%20%7C%20Digital%20Dreamer&descAlignY=55&descSize=18)

[![Typing SVG](https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=22&duration=3000&pause=1000&color=36BCF7&center=true&vCenter=true&width=600&lines=Building+Modern+Web+Applications;Passionate+About+Clean+Code;Always+Learning+New+Technologies;Let's+Create+Something+Amazing!)](https://git.io/typing-svg)

</div>

---

## 🚀 About Me

```typescript
const hamkarifai = {
    role: "Frontend Developer",
    location: "Yogyakarta, Indonesia 🇮🇩",
    code: ["TypeScript", "JavaScript", "Python"],
    focus: "Building modern, fast, and scalable web applications",
    motto: "Great code is not just written — it's crafted",
    currentlyLearning: ["Advanced Next.js patterns", "System Design"],
    funFact: "I believe every pixel matters ✨"
};
```

I specialize in crafting **pixel-perfect, performant web experiences** that users love. From building **fullstack Next.js applications** to designing **responsive UI components**, I transform ideas into reality with code that's not just functional—but elegant.

---

## 🛠️ Tech Arsenal

<div align="center">

### Frontend Powerhouse
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=next.js&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript&logoColor=white)
![Vite](https://img.shields.io/badge/Vite-646CFF?style=for-the-badge&logo=vite&logoColor=white)
![TailwindCSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white)

### Backend & Runtime
![Bun](https://img.shields.io/badge/Bun-000000?style=for-the-badge&logo=bun&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)

### Tools & Workflow
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS_Code-007ACC?style=for-the-badge&logo=visual-studio-code&logoColor=white)

</div>

---

## 💎 Core Competencies

<table>
<tr>
<td width="50%" valign="top">

### 🎯 Frontend Excellence
- ⚡ **Next.js Mastery** - SSR, SSG, ISR, API Routes
- 🎨 **UI/UX Implementation** - Pixel-perfect responsive designs
- 📱 **Mobile-First Development** - Progressive enhancement
- ♿ **Accessibility** - WCAG compliant interfaces
- 🔄 **State Management** - Context API, Zustand, TanStack Query

</td>
<td width="50%" valign="top">

### 🚀 Performance & Quality
- ⚙️ **TypeScript Expertise** - Type-safe, maintainable code
- 🎭 **Modern Tooling** - Vite, Bun, ESBuild
- 🧪 **Testing** - Unit & integration testing
- 📦 **Bundle Optimization** - Code splitting, lazy loading
- 🔧 **Clean Architecture** - Scalable project structures

</td>
</tr>
</table>

---

## 🏆 Featured Projects

<div align="center">

<table>
<tr>
<td width="50%">

### 🌐 Next.js Fullstack Platform
**Tech:** Next.js 14, TypeScript, Tailwind CSS

Fullstack application featuring authentication, REST API integration, server components, and responsive design. Built with performance and scalability in mind.

🔗 [View Project](#) | ⭐ Star

</td>
<td width="50%">

### ⚡ Lightning Todo App
**Tech:** Vite, React, Bun, TypeScript

Blazing fast todo application showcasing Bun's performance with sub-second hot module replacement and optimized production builds.

🔗 [View Project](#) | ⭐ Star

</td>
</tr>
<tr>
<td width="50%">

### 🎨 Component Library
**Tech:** React, TypeScript, Storybook

Reusable UI component library with Tailwind CSS, featuring comprehensive documentation and accessibility standards.

🔗 [View Project](#) | ⭐ Star

</td>
<td width="50%">

### 🔥 Modern Dashboard
**Tech:** Next.js, TanStack Query, Recharts

Analytics dashboard with real-time data visualization, dark mode support, and seamless API integration.

🔗 [View Project](#) | ⭐ Star

</td>
</tr>
</table>

</div>

---

## 📊 GitHub Analytics

<div align="center">
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=hamkarifai&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true&hide_border=true&bg_color=0D1117"/>
  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=hamkarifai&layout=compact&langs_count=8&theme=tokyonight&hide_border=true&bg_color=0D1117"/>
</div>

<div align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=hamkarifai&theme=tokyonight&hide_border=true&background=0D1117" />
</div>

<div align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=hamkarifai&theme=tokyonight&no-frame=true&no-bg=true&row=1&column=7" width="100%" />
</div>

---

## 🐍 Contribution Graph

<div align="center">
  
![Snake animation](https://github.com/hamkarifai/hamkarifai/blob/output/github-contribution-grid-snake.svg)

</div>

---

## 📈 Coding Activity

<!--START_SECTION:waka-->
<!--END_SECTION:waka-->

---

## 🌐 Let's Connect

<div align="center">

[![Email](https://img.shields.io/badge/Email-hamkarifairifai49%40gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:hamkarifairifai49@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Muhammad%20Hamka-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/muhammadhamkarifai)
[![Portfolio](https://img.shields.io/badge/Portfolio-Visit%20Website-00C7B7?style=for-the-badge&logo=google-chrome&logoColor=white)](#)
[![GitHub](https://img.shields.io/badge/GitHub-Follow%20@hamkarifai-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/hamkarifai)

</div>

---

<div align="center">

### 💭 Developer Quote

![Quote](https://quotes-github-readme.vercel.app/api?type=horizontal&theme=tokyonight)

### 😄 Random Dev Meme

<img src="https://readme-jokes.vercel.app/api?theme=tokyonight" alt="Jokes Card" />

---

### 👀 Profile Views

![Visitor Count](https://profile-counter.glitch.me/hamkarifai/count.svg)

<br>

**⭐ From [hamkarifai](https://github.com/hamkarifai) with 💙**

*"Code is poetry, and every commit is a verse."*

</div>

![Footer](https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=12,14,18,20,24&height=120&section=footer)
